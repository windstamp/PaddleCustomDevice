...
static void ReduceDims(const Context& dev_ctx,
                       ...
                       const phi::DDim& brd_ddims, const phi::DenseTensor& in,
                       phi::DenseTensor* out) {
...
template <typename T, typename Context>
void ElementwiseMulKernel(const Context& dev_ctx,
...
template <typename T, typename Context>
void ElementwiseMulGradKernel(const Context& dev_ctx,
                    const phi::DenseTensor& x,
                    const phi::DenseTensor& y,
  ...
  auto stream = dev_ctx.stream();
    ...
    if (dx) {
      if (dx->dims() == dout.dims()) {
        ...
        runner_dx.Run(stream);
        ReduceDims<T>(dev_ctx, stream, axis, dx->dims(), trans_x.dims(), dx_temp,
                      dx);
      }
    }
    if (dy) {
      if (dy->dims() == dout.dims()) {
        ...
        runner_dy.Run(stream);
        ReduceDims<T>(dev_ctx, stream, axis, dy->dims(), trans_y.dims(), dy_temp,
                      dy);
      }
    }
}
...
PD_REGISTER_PLUGIN_KERNEL(
    elementwise_mul, ascend, ALL_LAYOUT, custom_kernel::ElementwiseMulKernel, float,
#ifdef PADDLE_WITH_ASCEND_INT64
    int64_t,
#endif
    double) {}

PD_REGISTER_PLUGIN_KERNEL(elementwise_mul_grad,
                          ascend,
                          ALL_LAYOUT,
                          custom_kernel::ElementwiseMulGradKernel,
                          float,
#ifdef PADDLE_WITH_ASCEND_INT64
                          int64_t,
#endif
