...
void ElementwiseSubKernel(const Context& dev_ctx,
...
template <typename T, typename Context>
void ElementwiseSubGradKernel(const Context& dev_ctx,
  ...
  auto stream = dev_ctx.stream();
    ...
    if (dx) {
      ...
      if (axes.size() != 0) {
        ...
        phi::DenseTensorMeta reduced_dout_meta = {dx->dtype(), phi::make_ddim(reduced_dout_dims)};
    ...
    if (dy) {
      ...
      if (axes.size() != 0) {
        ...
        phi::DenseTensorMeta reduced_dout_meta = {dy->dtype(), phi::make_ddim(reduced_dout_dims)};
        reduced_dout.set_meta(reduced_dout_meta);
        dev_ctx.template Alloc<T>(&reduced_dout);
...
PD_REGISTER_PLUGIN_KERNEL(
    elementwise_sub, ascend, ALL_LAYOUT, custom_kernel::ElementwiseSubKernel, float,
#ifdef PADDLE_WITH_ASCEND_INT64
    int64_t,
#endif
    double) {}

PD_REGISTER_PLUGIN_KERNEL(elementwise_sub_grad,
                          ascend,
                          ALL_LAYOUT,
                          custom_kernel::ElementwiseSubGradKernel,
                          float,
#ifdef PADDLE_WITH_ASCEND_INT64
                          int64_t,
#endif
