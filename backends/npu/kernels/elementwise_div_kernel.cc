...
void DivideGradKernel(const Context& dev_ctx,
  ...
  if (dx) {
    ...
    // LOG(WARNING) << "tensor_one.dtype(): " << tensor_one.dtype();
    // LOG(WARNING) << "y.dtype(): " << y.dtype();
    // LOG(WARNING) << "y_div.dtype(): " << y_div.dtype();
  ...
  if (dy) {
    ...
    if (false) {
      ...
      // LOG(WARNING) << "neg_out.dtype(): " << neg_out.dtype();
      // LOG(WARNING) << "y.dtype(): " << y.dtype();
      // LOG(WARNING) << "y_grad_w.dtype(): " << y_grad_w.dtype();
    ...
    // fix elementwise_div_grad for model PPO
    // PR: https://github.com/PaddlePaddle/Paddle/pull/42484
    if (true) {
      phi::DenseTensor neg_out;
