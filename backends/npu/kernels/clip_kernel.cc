...
void ClipKernel(const Context& dev_ctx,
  ...
  PADDLE_ENFORCE_LE(
      min_,
      max_,
      phi::errors::InvalidArgument("max should be greater than or equal to min. "
  ...
  FillNpuTensorWithConstant<T>(&max_tensor, dev_ctx, min_);

  phi::DenseTensor min_tensor;
  min_tensor.Resize(phi::make_ddim({1}));
  FillNpuTensorWithConstant<T>(&min_tensor, dev_ctx, max_);
...
PD_REGISTER_PLUGIN_KERNEL(
    clip, ascend, ALL_LAYOUT, custom_kernel::ClipKernel, float, double) {}
