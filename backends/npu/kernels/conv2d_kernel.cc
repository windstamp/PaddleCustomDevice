...
void DepthwiseConv2dKernel(const Context& dev_ctx,
  ...
  if (channel_last) {
    ...
    phi::DenseTensorMeta transformed_filter_meta = {
          filter.dtype(), {filter.dims()[1], filter.dims()[0],
...
void DepthwiseConv2dGradKernel(const Context& dev_ctx,
  ...
  const bool channel_last = data_format == "NHWC";
    ...
    std::vector<int> ksize = phi::vectorize<int>(filter_data_dims);
    UpdatePaddingAndDilation(&paddings, &dilation, padding_algorithm,
                             in_data_dims, stride, ksize);
    ...
    if (input_grad) {
      ...
      if (channel_last) {
        phi::DenseTensorMeta input_grad_meta = {
          input_grad->dtype(), input_grad->dims()};
        input_grad_tensor.set_meta(input_grad_meta);
      }
      NpuOpRunner runner;
...
PD_REGISTER_PLUGIN_KERNEL(
    depthwise_conv2d, ascend, ALL_LAYOUT, custom_kernel::DepthwiseConv2dKernel, float) {}
PD_REGISTER_PLUGIN_KERNEL(
    depthwise_conv2d_grad, ascend, ALL_LAYOUT, custom_kernel::DepthwiseConv2dGradKernel, float) {}
