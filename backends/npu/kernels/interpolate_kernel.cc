...
void NearestInterpKernel(const Context& dev_ctx,
  ...
  auto stream = dev_ctx.stream();
    ...
    ExtractNCDWH(input_dims, data_layout, &n, &c, &in_d, &in_h,
                             &in_w);
    ...
    if (size_tensor && size_tensor->size() > 0) {
      ...
      auto out_size_data = get_new_data_from_tensor<int>(out_size.get_ptr());
      out_h = out_size_data[0];
      out_w = out_size_data[1];
    } else {
      if (scale_tensor) {
        auto scale_data = get_new_data_from_tensor<float>(scale_tensor.get_ptr());
    ...
    //   BilinearFwdNpu<T>(ctx, input, out, scale_h, scale_w, align_corners,
    //                     align_mode, data_layout);
    }
}

template <typename T, typename Context>
void NearestInterpGradKernel(const Context& dev_ctx,
                    const phi::DenseTensor& x,
                    paddle::optional<const phi::DenseTensor&> out_size,
                    paddle::optional<const std::vector<const phi::DenseTensor*>> size_tensor,
                    paddle::optional<const phi::DenseTensor&> scale_tensor,
  ...
  auto stream = dev_ctx.stream();
    ...
    phi::DataLayout data_layout = StringToDataLayout(data_layout_str);
    int n, c, in_d, in_h, in_w;
    ExtractNCDWH(input.dims(), data_layout, &n, &c, &in_d, &in_h,
                             &in_w);
    ...
    if (size_tensor && size_tensor->size() > 0) {
      ...
      if (scale_tensor) {
        auto scale_data =
            get_new_data_from_tensor<float>(scale_tensor.get_ptr());
        if (scale_data.size() > 1) {
          scale_h = scale_data[0];
          scale_w = scale_data[1];
        } else {
    ...
    phi::DenseTensorMeta input_grad_meta = {x.dtype(), dim_grad};
...
PD_REGISTER_PLUGIN_KERNEL(
    nearest_interp_v2, ascend, ALL_LAYOUT, custom_kernel::NearestInterpKernel,
    uint8_t,
    int,
    int64_t,
    float,
