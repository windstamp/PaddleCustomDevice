...
void UnsqueezeKernel(const Context& dev_ctx,
                     ...
                     phi::DenseTensor* out) {
  LOG(WARNING) << "!!!!!!!!!!!!! here !!!!!!!!!!!!!";
  // LOG(WARNING) << "x.place(): " << x.place();
  // LOG(WARNING) << "x.dims(): " << x.dims();
  auto x_dims = x.dims();
  // auto out_dims = out->dims();
  // auto out_dims = x.dims();
  auto out_dims = phi::make_ddim({1, 3, 1, 40});
  if (axes.FromTensor()) {
    LOG(WARNING) << "!!!!!!!!!!!!! here !!!!!!!!!!!!!";
  ...
  // LOG(WARNING) << "out->place(): " << out->place();
...
void UnsqueezeGradKernel(const Context& dev_ctx,
                         ...
                         phi::DenseTensor* dx) {
  LOG(WARNING) << "!!!!!!!!!!!!! here !!!!!!!!!!!!!";
  // LOG(WARNING) << "x_shape.place(): " << x_shape.place();
  LOG(WARNING) << "x_shape.dims(): " << x_shape.dims();
  LOG(WARNING) << "dout.place(): " << dout.place();
  LOG(WARNING) << "dout.dims(): " << dout.dims();

  auto xshape_dims = x_shape.dims();
  auto x_dims = phi::slice_ddim(xshape_dims, 1, xshape_dims.size());
  LOG(WARNING) << "x_dims: " << x_dims;
...
//     unsqueeze, ascend, ALL_LAYOUT, custom_kernel::UnsqueezeKernel,
//     bool,
//     int,
//     int8_t,
