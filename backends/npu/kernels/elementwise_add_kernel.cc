...
void ElementwiseAddKernel(const Context& dev_ctx,
...
template <typename T, typename Context>
void ElementwiseAddGradKernel(const Context& dev_ctx,
                    const phi::DenseTensor& x,
                    const phi::DenseTensor& y,
    ...
    if (dx) {
      ...
      if (dx->dims() != dout.dims()) {
        ...
        if (!reduce_axes.empty()) {
          ...
          phi::DenseTensorMeta tmp_meta = {dx->dtype(), phi::make_ddim(dst_dims_vec)};
    ...
    if (dy) {
      ...
      if (dy->dims() != dout.dims()) {
        ...
        if (!reduce_axes.empty()) {
          ...
          phi::DenseTensorMeta tmp_meta = {dy->dtype(), phi::make_ddim(dst_dims_vec)};
          tmp.set_meta(tmp_meta);
          const auto& runner =
              NpuOpRunner("ReduceSumD", {dout}, {tmp},
                          {{"axes", reduce_axes}, {"keep_dims", false}});
...
PD_REGISTER_PLUGIN_KERNEL(
    elementwise_add, ascend, ALL_LAYOUT, custom_kernel::ElementwiseAddKernel, float,
#ifdef PADDLE_WITH_ASCEND_INT64
    int64_t,
#endif
    double) {}

PD_REGISTER_PLUGIN_KERNEL(elementwise_add_grad,
                          ascend,
                          ALL_LAYOUT,
                          custom_kernel::ElementwiseAddGradKernel,
                          float,
#ifdef PADDLE_WITH_ASCEND_INT64
                          int64_t,
#endif
